...
import com.clinic.model.exceptions.BusinessException;
...
public class OrderService {
    ...
    public Order createOrder(Patient patient, User doctor, List<OrderItem> items) {
        ...
        if (doctor == null) {
            throw new BusinessException("El doctor no puede ser null");
        }

        // 2. Validar que el rol sea DOCTOR
        if (doctor.getRole() != Role.DOCTOR) {
            throw new BusinessException("Solo un doctor puede crear ordenes");
        }

        // 3. Validar paciente
        if (patient == null) {
            throw new BusinessException("El paciente no puede ser null");
        }
